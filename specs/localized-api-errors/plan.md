...
- [x] 6. Call-site migration (depends on: 2, 5, 7)
  ...
  - [x] 6.5 Verify against a locally running API that a wrong-password sign-in shows the localized message, and that each migrated site still shows a localized message when the API returns no code
        Verified against the real stack (dockerised Postgres + `npm run dev` API on :3030 + vite on :4173, which is already an allowed CORS origin). A wrong-password sign-in renders "Email-Adresse oder Passwort ist nicht korrekt." and never "Invalid login"; an unregistered email renders the identical message (no enumeration in the UI); an uncoded 500 renders the localized 5xx message with the server's internal text absent. Curl against the live API also confirmed the wrong-password and unknown-email bodies are byte-identical and that the 429 body is Feathers-shaped JSON carrying `RATE_LIMITED`. Driven by a throwaway Playwright spec, deleted after the run.
