...
const reactivateUserEntries = async (id) => {
  await BaseModel.knex().raw(`
    update farms f set active = true where f.id IN
  ...
  await BaseModel.knex().raw(`
    update initiatives i set active = true where i.id IN
  ...
  await BaseModel.knex().raw(`
    update depots d set active = true where d.id IN
...
export const reactivateUser = async (app, id) => {
  ...
  await reactivateUserEntries(id)
}
