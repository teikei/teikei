...
class Api::V1::MessagesController < ApplicationController
  ...
  def create
    if params.has_key? :place_form
      ...
      render json: {errors: "Missing form data."}, status: 422
    end
  end
  ...
  private
    ...
    def create_place_message(form_data)
      begin
        ...
        render json: {errors: "Invalid recipient. Place not found."}, status: 422
        return
      end
      ...
      if message.deliver
        render json: {errors: "Successfully sent message to #{place.contact_name}."}, status: 201
      else
        render json: {errors: "Failure sending message to #{place.contact_name}."}, status: 401
      end
    end
